...
if ($action === 'delete') {
    ...
    $select_query = "SELECT file FROM deped_order WHERE id = ?";
    ...
    $delete_query = "DELETE FROM deped_order WHERE id = ?";
...
if ($action === 'update' && $id > 0) {
    ...
    $query = "UPDATE deped_order SET title = ?, date = ?, description = ?";
    ...
    $query = "INSERT INTO deped_order (title, date, description, file) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($query);
