...
$create_table = "CREATE TABLE IF NOT EXISTS deped_memorandum (
...
if ($action === 'delete') {
    ...
    $select_query = "SELECT file FROM deped_memorandum WHERE id = ?";
    ...
    $delete_query = "DELETE FROM deped_memorandum WHERE id = ?";
...
if ($action === 'update' && $id > 0) {
    ...
    $query = "UPDATE deped_memorandum SET title = ?, date = ?, description = ?";
    ...
    $query = "INSERT INTO deped_memorandum (title, date, description, file) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($query);
