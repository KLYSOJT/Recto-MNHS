...
if ($action === 'delete') {
    ...
    $select_query = "SELECT file FROM school_memorandum WHERE id = ?";
    ...
    $delete_query = "DELETE FROM school_memorandum WHERE id = ?";
...
if ($action === 'update' && $id > 0) {
    ...
    $query = "UPDATE school_memorandum SET title = ?, date = ?, description = ?";
    ...
    $query = "INSERT INTO school_memorandum (title, date, description, file) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($query);
